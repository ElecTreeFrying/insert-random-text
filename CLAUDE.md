...
- **Prompted commands.** The parameterized types (`numberRange` / `floatRange` / `stringLength` / `dateBetween` / `wordsCount` / `sentencesCount` / `paragraphsCount` / `uuidFormat` / `passwordOptions` / `phoneFormat`, all `insertRandomText.*`) ask for parameters in input boxes and Quick Picks, then insert via `insertWith`. Adding one: entry in `promptedCommands` (`src/prompted.ts`) + a `package.json` command `insertRandomText.<id>` — registration is automatic; they are intentionally **not** in `COMMAND_TO_GENERATOR` (the parity test reads `promptedCommands` and holds contributed ↔ registered in sync, and guards prompted ids against catalog-id collisions).
