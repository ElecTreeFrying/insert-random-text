...
- **Pure areas, headless** — `npm run compile-tests && npx mocha "out/test/{quote,config,catalog,engine,format,record,prompted,custom}/*.test.js"`. Don't widen the glob to `**`: `extension.test.ts` and `commands/` import `vscode` and fail under plain node.
...
Ten single-responsibility modules. Generation is `vscode`-free and lives apart from the editor glue, so the logic that matters is decoupled from the API surface:
