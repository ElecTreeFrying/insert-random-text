...
| **Insert Random: Number (Range…) / Float (Range…) / String (Length…) / Date (Between…) / Words (Count…) / Sentences (Count…) / Paragraphs (Count…)** | Parameterized types — enter a min & max, a length, a from/to date, or a lorem count in input boxes and the value is generated to your spec, through the same pipeline (multi-cursor, bulk, quoting, seed). Your last inputs are remembered and prefilled; Esc cancels cleanly. |
