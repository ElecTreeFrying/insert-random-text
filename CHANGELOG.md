...
- **Parameterized types** — new *Insert Random: Number (Range…)*, *Float (Range…)*, *String (Length…)*, and *Date (Between…)* commands ask for a min & max, a length up to 1000, or a from/to date (`YYYY-MM-DD` or full ISO 8601) in validated input boxes, then insert through the normal pipeline (multi-cursor, bulk, quoting, seed all apply). Last-used inputs are remembered and prefilled; Esc cancels cleanly.
