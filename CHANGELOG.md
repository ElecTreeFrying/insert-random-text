...
- **Anonymize in place** — new *Insert Random: Randomize Selection* command replaces each selected text with a same-shape randomization: digits become digits, letters keep their case, punctuation and layout don't move (`3.14` → `8.77`, `Bob@x.io` → `Kqe@v.zt`). Works across a multi-selection in one step, honors the seed for reproducible scrubs, and joins the editor right-click submenu.
